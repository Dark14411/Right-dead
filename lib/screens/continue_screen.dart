...
class ContinueScreen extends PositionComponent
    with TapCallbacks, HasGameRef<FlameGame> {
  final VoidCallback onContinuePressed;
  late SpriteComponent continueBackground;

  ContinueScreen({required this.onContinuePressed});
  ...
  Future<void> onLoad() async {
    ...
    // Cargar la imagen de continuar
    final sprite = await Sprite.load('Continuar.png');
    ...
    continueBackground = SpriteComponent(
      sprite: sprite,
      size: displaySize,
      position: displayPosition,
    );

    add(continueBackground);
  }

  @override
  void onTapDown(TapDownEvent event) {
    // Al hacer clic en cualquier parte de continuar, ejecutar la acción
    onContinuePressed();
  }
  ...
  void onGameResize(Vector2 newSize) {
    ...
    if (continueBackground.sprite != null) {
      final imageSize = continueBackground.sprite!.srcSize;
      ...
      continueBackground.size = displaySize;
      continueBackground.position = Vector2(
